...
describe('Scenario 3 – Remove events from activity log:', () => {
    ...
    it('should ', () => {
        ...
        cy.get('.panel-subheader .listview-status .text-number:not([id*=Count])').then(element => {
            cy.wrap(Number(element.text().replace(/,/g, ''))).as('numberOfActivities');
        });
        ...
        cy.get('.panel-subheader .listview-status .text-number:not([id*=Count])').then(element => {
            cy.wrap(Number(element.text().replace(/,/g, ''))).as('numberOfActivitiesNew');
        });
    ...
    it('should ', function () {
        ...
        cy.wrap(this.numberOfActivitiesNew).should('eq', this.numberOfActivities - 0);
    });
})
